##################################################################################################################
#
#   PyNAFF  -  A Python module for calculating the Fundamental Frequencies of Quasi-Periodic Signals
...
__version__       = 1.0
__authors__       = ["Nikos Karastathis (nkarast .at. cern .dot. ch)", "Panagiotis Zisopoulos (pzisopou .at. cern .dot. ch)"]
__pythonVersion__ = '2.7.11+'
...
class PyNAFF(object):
    ...
    """PyNAFF class()
        ...
        -- Functions of the Class:
            ...
            * run()
    """
    def __init__(self, filename='track.obs0001.p0001', isFile=True, inputParamFile=None, saveFiles=False, solutionFile='track.sol', parametersOutLog='nafpar.par',
        outBuffFile='naf.out', tabFile='naf.tab', KTABS=300, batch=False, debug=False, logfile=None, loglevel=10, neps=100000000.,
        nterm=1, icplx=1, iw=+1, nulin=0, icol=1, icx=1,icy=1, iprnaf=0, iftes=0, ifsav=0, ifcomp=0, ifres=0, ifstab=1,imodt=0):
        ...
        if not isFile:
            self.result             = []                                    # list to keep the results to be returned

        self.UNIANG = None                                          # unit of the angle = 1rd = 1 uniang unit of the angle (if the unit is in seconds of the arc then UNIANG = 206264.80624709)
        self.FREFON = None                                          # Fundamental Frequency : FREFON = 2*pi/(KTABS*XH) or in seconds FREFON = 360*3600 / (KTABS*XH)
        self.EPS    = None                                          # Precision
        self.IKT    = None                                          # Step
        self.AF = None                                              # Real part of amplitude
        self.BF = None                                              # Imaginary part of amplitude
...
# - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - -
    def initLogger(self, logfile, loglevel):
...
# - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - -
    def setParametersFilename(self, paramsFname):
        '''Accessor function
        '''
...
# - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - -
    def setDataFilename(self, dataFname):
        '''Accessor function
        '''
        self.dataFilename = dataFname
# - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - -
    def setSolutionFilename(self, solFname):
        '''Accessor function
        '''
        self.solutionFilename = solFname
# - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - -
    def setTabFilename(self, tabFname):
        '''Accessor function
        '''
        self.tabFilename = tabFname
# - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - -
    def setDTOUR(self, nDTOUR):
        '''Accessor function
        '''
...
# - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - -
    def setXH(self, nXH):
        ...
        self.XH = nXH
# - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - -
    def setKTABS(self, nKTABS):
        '''Accessor function
        '''
        self.KTABS = int(nKTABS)
# - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - -
    def setNEPS(self, nNEPS):
        '''Accessor function
        '''
        self.NEPS = nNEPS
# - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - -
    def setNTERM(self, nNTERM):
        '''Accessor function
        '''
        self.NTERM = nNTERM
# - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - -
    def setICPLX(self, nICPLX):
        '''Accessor function
        '''
        self.ICPLX = nICPLX
# - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - -
    def setIW(self, nIW):
        ...
        self.IW = nIW
# - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - -
    def setNULIN(self, nNULIN):
        '''Accessor function
        '''
        self.NULIN = nNULIN
# - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - -
    def setICOL(self, nICOL):
        '''Accessor function
        '''
        self.ICOL = nICOL
# - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - -
    def setICX(self, nICX):
        '''Accessor function
        '''
        self.ICX = nICX
# - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - -
    def setICY(self, nICY):
        '''Accessor function
        '''
        self.ICY = nICY
# - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - -
    def setIPRNAF(self, nIPRNAF):
        '''Accessor function
        '''
        self.IPRNAF = nIPRNAF
# - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - -
    def setIFTES(self, nIFTES):
        '''Accessor function
        '''
        self.IFTES = nIFTES
# - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - -
    def setIFSAV(self, nIFSAV):
        '''Accessor function
        '''
        self.IFSAV = nIFSAV
# - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - -
    def setIFCOMP(self, nIFCOMP):
        '''Accessor function
        '''
        self.IFCOMP = nIFCOMP
# - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - -
    def setIFRES(self, nIFRES):
        '''Accessor function
        '''
        self.IFRES = nIFRES
# - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - -
    def setIFSTAB(self, nIFSTAB):
        '''Accessor function
        '''
        self.IFSTAB = nIFSTAB
# - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - -
    def setIMODT(self, nIMODT):
        '''Accessor function
        '''
        self.IMODT = nIMODT
# - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - -
    def setTMF1(self, nTMF1_arr):
        '''Accessor function
        '''
        self.TMF1 = nTMF1_arr
# - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - -
    def setTMF2(self, nTMF2_arr):
        '''Accessor function
        '''
        self.TMF2 = nTMF2_arr
# - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - -
    def setTNT(self, nTNT_arr):
        '''Accessor function
        '''
        self.TNT = nTNT_arr
# - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - -
    def readParamsFile(self, paramsfile='nafaut.par'):
        ...
        f.close()
# - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - -
    def printConfig(self, retString=False):
        ...
        TMF1                    = {}
        TMF2                    = {}
        TNT                     = {}'''.format(self.parametersFilename, self.outputFilename, self.dataFilename, self.solutionFilename, self.tabFilename, self.DTOUR, self.T0, self.XH, self.KTABS, self.NEPS, self.NTERM, self.ICPLX, self.IW, self.NULIN, self.ICOL, self.ICX, self.ICY, self.IPRNAF, self.IFTES, self.IFSAV, self.IFCOMP, self.IFRES, self.IFSTAB, self.IMODT, self.TMF1, self.TMF2, self.TNT)
        info(c)
        if retString:
            return c
# - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - -
    def inipar(self, fromFile=None):
...
# - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - -
    def loadArray(self, KTABS, ICX, ICY, ICOL, NULIN, filename):
        ...
        Returns: None
        '''
        if filename.dtype != 'complex':
            filename = filename.astype('complex128')

        ICX = ICX-1
        ICY = ICY-1

        if len(filename.shape) == 1:
            self.ZTABS[:KTABS+1] = filename[NULIN:NULIN+KTABS+1]
        else:
            raise IndexError("# loadArray : ndarray input not yet implemented.")
# - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - -
    def loadsol(self, KTABS, ICX, ICY, ICOL, NULIN, filename):
...
# - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - -
    def prtabs(self, KTABS, ZTABS, IPAS, where=0):
...
# - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - -
    def fftmax(self, KTABS):
        ...
        return FR
# - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - -
    def frefin(self, KTABS, FR,  RPASS0, RPREC):
        ...
        NFPRT = None
        IPRT = 1
        XM, YM = self.maxiqua(X, PASS, EPS, IPRT, NFPRT)
        ...
        debug("# frefin returns to MFTNAF : FR = {}, A = {}, B = {}, RM = {}".format(FR, A, B, RM))

        return FR, A, B, RM
# - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - -
    def maxiqua(self, X, PASS, EPS, IPRT, NFPRT):
...
# - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - -
    def profre(self, KTABS, FS):
        ...
        ZT, ZTF = self.ztpow2(int(self.KTABS+1), 64, self.ZTABS, self.TWIN, ZINC, ZEX)
        ...
        return RMD, A, B
# - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - -
    def ztpow2(self, N, N1, ZTA, TW, ZA, ZAST):
...
# - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - -
    def ztpow(self, N, N1, ZA, ZAST):
        ...
        return ZT
# - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - -
    def ztpow2a(self, N, N1, TW, ZA, ZAST):
...
# - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - -
    def fretes(self, FR, TOL):
        ...
        return IFLAG, NUMFR
# - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - -
    def proscaa(self, KTABS, F1, F2): # ZP is the return value
        ...
        ZT, ZTF = self.ztpow2a(KTABS+1, 64,  self.TWIN, ZINC, ZEX)
        ## STEP SIZE
        H = np.float64(1.0)/LTF
        ...
        for i in xrange(0,NF):
            ...
            if self.ICPLX == 1:
                ...
                for IT in xrange(0, int(self.KTABS)+1):
                    self.ZTABS[IT] = self.ZTABS[IT]-ZT[IT]
# - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - -
    def modfre(self, KTABS, NUMFR, A,B ): #(KTABS,NUMFR,A,B)
        ...
        debug("# modfre : Returning ZT = {}".format(ZT))
# - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - -
    def mftnaf(self, KTABS, NTERM, EPS):
...
# - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - -
    def appendToSol(self, NFSOLFILENAME, mystr):
        ...
        fsol.close()
# - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - -
    def tessol(self, KTABS, EPS):  # no idea what this one does
        ...
        return TFSR, ZAMPR
# - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - -
    def compso(self, NFOUTFILE, NFS, TFSA, ZAMPA, TFSB, ZAMPB):
        ...
        if NFOUTFILE == "STDOUT":
            ...
            fout.close()
# - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - -
    def savtab(self, KTABS, ZTABS, nomftab):
...
# - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - - * - - -
    def run(self):
        ...
        if self.isFile:
            ...
            self.loadArray(self.KTABS, self.ICX, self.ICY, self.ICOL, self.NULIN, filename=self.dataFilename)
        ...
        if (self.IFSTAB == 1) and (self.saveFiles):
            self.savtab(self.KTABS, self.ZTABS, self.tabFilename)
...
def naff_file(data='track.obs0001.p0001', turns=300, nterm=1, icol=1, icx=1, icy=0, saveFiles=False, doAll=False, neps=100000000., loglevel=20, logfile=None, debug=False, batch=True,
    ...
    n = PyNAFF(KTABS=turns, filename=data, saveFiles=saveFiles, isFile=True, inputParamFile=inputParamFile, solutionFile=solutionFile, parametersOutLog=parametersOutLog,
...
def naff_array(data=None, turns=300, nterm=1, icol=1, icx=1, icy=0, saveFiles=False, doAll=False, neps=100000000., loglevel=20, logfile=None, debug=False, batch=True,
         icplx=1, iw=+1, nulin=0,  iprnaf=0, iftes=0, ifsav=0, ifcomp=0, ifres=0, ifstab=1,imodt=0, inputParamFile=None, solutionFile='track.sol',
         parametersOutLog='nafpar.par', outBuffFile='naf.out', tabFile='naf.tab'):
    ...
    n = PyNAFF(KTABS=turns, filename=data, saveFiles=saveFiles, isFile=False, inputParamFile=inputParamFile, solutionFile=solutionFile, parametersOutLog=parametersOutLog,
        outBuffFile=outBuffFile, tabFile=tabFile, batch=batch, debug=debug, logfile=logfile, loglevel=loglevel, neps=neps,
        nterm=nterm, icplx=icplx, iw=iw, nulin=nulin, icol=icol, icx=icx ,icy=icy, iprnaf=iprnaf, iftes=iftes, ifsav=ifsav, ifcomp=ifcomp, ifres=ifres, ifstab=ifstab,imodt=imodt)
...
def naff(data='track.obs0001.p0001', turns=300, nterm=1, icol=1, icx=1, icy=0, saveFiles=False, doAll=False, neps=100000000., loglevel=20, logfile=None, debug=False, batch=True,
         icplx=1, iw=+1, nulin=0,  iprnaf=0, iftes=0, ifsav=0, ifcomp=0, ifres=0, ifstab=1,imodt=0, inputParamFile=None, solutionFile='track.sol',
         parametersOutLog='nafpar.par', outBuffFile='naf.out', tabFile='naf.tab'):
